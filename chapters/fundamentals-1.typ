...
- *Kognitive Intensität:* Wissensarbeit ist durch nicht-routinemäßige, kognitiv anspruchsvolle Tätigkeiten gekennzeichnet, die auf dem Aufbau, der Anwendung und der Verknüpfung spezialisierten intellektuellen Wissens beruhen (vgl. @DellAcqua2026[S. 406]).
- *Autonomie und Verantwortung:* Wissensarbeitende entscheiden innerhalb definierter Grenzen autonom über die Herangehensweise an eine Aufgabe und tragen die Verantwortung für die Produktivität ihrer Arbeit (vgl. @drucker1999knowledge[S. 84, S. 86]).
- *Kontinuierliches Lernen:* Wissensarbeit erfordert die fortwährende Bereitschaft, bestehendes Wissen zu aktualisieren und neue Fähigkeiten zu erwerben, da sich die Anforderungen in digitalisierten Arbeitsumgebungen kontinuierlich verändern (vgl. @Drucker1994AgeOfSocialTransformation).
...
+ *Diskontinuierliche technologische Grenze (Jagged Frontier):* KI-Systeme zeigen in kognitiven Domänen eine ungleichmäßige Leistung. Während sie bei kreativen Aufgaben oft menschliche Experten übertreffen, können sie bei logisch ähnlich komplexen Aufgaben unvorhersehbar scheitern (vgl. @DellAcqua2026[S. 403]).
...
+ *Validierungsaufwand:* Da LLMs zu Halluzinationen und inhaltlich falschen, aber plausibel klingenden Ausgaben neigen, steigt der Aufwand für Verifizierungsprozesse (vgl. @Lee2025[S. 15]; @Brynjolfsson2025[S. 897]; @DellAcqua2026[S. 404]).
...
Large Language Models (LLMs) zählen zu den bedeutendsten Entwicklungen innerhalb der jüngeren KI-Forschung (vgl. @Bubeck2023SparksAGI[S. 4]). Es handelt sich dabei um neuronale Netzwerkmodelle, die anhand riesiger Mengen an Textdaten aus dem Internet trainiert werden. Das Modell lernt dabei, auf Basis eines gegebenen Kontextes die Wahrscheinlichkeit nachfolgender Tokens vorherzusagen (vgl. @Bubeck2023SparksAGI[S. 4]). Obwohl der Begriff menschliche Sprache impliziert, lassen sich diese Techniken auch auf andere Formen sequenzieller Daten anwenden, etwa Computercode, Proteinsequenzen oder Schachpartien (vgl. @Eloundou2023GPTsAreGPTs[S. 1]). Dabei bezeichnet der Begriff ein Modell, das auf extrem großen Datensätzen (oft im Tera- oder Petabyte-Bereich) trainiert wurde und über Milliarden von Parametern verfügt, um komplexe linguistische Strukturen abzubilden (vgl. @Woodruff2024[Fn. 1]).

*Technische Funktionsweise der Textgenerierung*

Der Kern der Textgenerierung basiert auf der *wahrscheinlichkeitsbasierten Vorhersage* des nächsten Wortes (bzw. Tokens) in einer Sequenz (vgl. @Brynjolfsson2025[S. 895]; @Chen2025[S. 14]). Der entscheidende Fortschritt gelang durch die Einführung von Transformer-Modellen, die einen sogenannten Attention-Mechanismus nutzen, der es dem System erlaubt, Wörtern innerhalb eines Textes unterschiedliche Relevanzgewichte zuzuweisen und so semantische Abhängigkeiten über weite Distanzen hinweg zu erfassen (vgl. @Chen2025[S. 14]; @Brynjolfsson2025[S. 896]). Anstatt expliziten linguistischen Regeln zu folgen, lernen diese Modelle durch statistische Korrelationen, welche Begriffe in einem bestimmten Kontext üblicherweise zusammen auftreten (vgl. @jiao2025generative[S. 1]; @Brynjolfsson2025[S. 896]).

#pagebreak()
*Einsatzmöglichkeiten in der Wissensarbeit*

Im Gegensatz zur diskriminativen KI, die primär Daten klassifiziert, erzeugt generative KI eigenständig neue Inhalte, die menschlichen Erzeugnissen in ihrer Qualität nahekommen (vgl. @Felten2023_Heterogeneity[S. 1]; @jiao2025generative[S. 2]). Aufgrund dieser Eigenschaften finden LLMs in der Wissensarbeit breite Anwendung, etwa bei der Texterstellung und Dokumentation, der Codegenerierung sowie der Zusammenfassung und Extraktion von Informationen aus umfangreichen Dokumenten (vgl. @Eloundou2023GPTsAreGPTs[S. 1-2]; @Noy2023ProductivityAI[S. 1-2]; @Chen2025[S. 14-15]).
